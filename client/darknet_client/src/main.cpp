...
int main(int argc, char *argv[])
{
  ...
  if (vid_input_flag) {
    ...
    out_vid_path = string(vid_path, strrchr(vid_path, '.')) + "_output.avi";
  }
  ...
  end_frame = cap.get(CV_CAP_PROP_FRAME_COUNT);
  ...
  if (vid_output_flag) {
    writer.open(out_vid_path,CV_FOURCC('X','V','I','D'), fps, Size(cap_width, cap_height), true);
    if (!writer.isOpened()) {
